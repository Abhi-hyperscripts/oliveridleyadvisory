...
return yPoint}
function fnOrVal(s){if(s[0]==="(")return eval(s)
else return parseFloat(s)}
lax.setup=(o={})=>{lax.breakpoints=o.breakpoints||{}
lax.selector=o.selector||'.lax'
lax.populateElements()}
lax.removeElement=(el)=>{const i=lax.elements.findIndex(o=>o.el=el)
if(i>-1){lax.elements.splice(i,1)}}
lax.createLaxObject=(el)=>{const o={el:el,originalStyle:{transform:el.style.transform,filter:el.style.filter},transforms:{}}
return o}
lax.calcTransforms=(o)=>{const{el}=o
const presets=[]
for(let i=0;i<el.attributes.length;i++){const a=el.attributes[i]
if(a.name.indexOf("data-lax-preset")>-1){presets.push(a);}}
for(let i=0;i<presets.length;i++){const a=presets[i]
const b=a.name.split(breakpointsSeparator)
const breakpoint=b[1]?`${breakpointsSeparator}${b[1]}`:''
a.value.split(" ").forEach((p)=>{const bits=p.split("-")
const fn=lax.presets[bits[0]]
if(!fn){console.log(`lax error: preset ${bits[0]} is not defined`)}else{const d=fn(bits[1])
for(let k in d){el.setAttribute(`${k}${breakpoint}`,d[k])}}})
...
if(useGpu){el.style["backface-visibility"]="hidden"
el.style["-webkit-backface-visibility"]="hidden"}
if(el.attributes["data-lax-use-gpu"])el.attributes.removeNamedItem("data-lax-use-gpu")
o.optimise=false
if(el.attributes["data-lax-optimize"]&&el.attributes["data-lax-optimize"].value==='true'){o.optimise=true
...
if(bits[1]==="lax"){if(a.name==="data-lax-anchor"){o["data-lax-anchor"]=a.value==="self"?el:document.querySelector(a.value)
const rect=o["data-lax-anchor"].getBoundingClientRect()
o.anchorTop=Math.floor(rect.top)+window.scrollY}else{const tString=a.value.replace(/vw/g,window.innerWidth).replace(/vh/g,window.innerHeight).replace(/elh/g,el.clientHeight).replace(/elw/g,el.clientWidth).replace(/\s+/g," ")
const[arrString,optionString]=tString.split("|")
const options={}
if(optionString){optionString.split(" ").forEach((o)=>{const[key,val]=o.split("=")
options[key]=key&&fnOrVal(val)})}
const name=b[0]
const valueMap=arrString.split(",").map((x)=>{return x.trim().split(" ").map(fnOrVal)}).sort((a,b)=>{return a[0]-b[0]})
if(!o.transforms[name]){o.transforms[name]={}}
...
if(spriteData){o.spriteData=spriteData.split(",").map(x=>parseInt(x))
...
if(spriteImage){el.style.backgroundImage=`url(%24%7bspriteImage%7d.html)`}}
return o}
lax.addElement=(el)=>{const o=lax.calcTransforms(lax.createLaxObject(el))
lax.elements.push(o)
lax.updateElement(o)}
lax.populateElements=()=>{lax.elements=[]
document.querySelectorAll(lax.selector).forEach(lax.addElement)
currentBreakpoint=lax.getCurrentBreakPoint()}
lax.updateElements=()=>{lax.elements.forEach((o)=>{lax.calcTransforms(o)
lax.updateElement(o)})
currentBreakpoint=lax.getCurrentBreakPoint()}
lax.getCurrentBreakPoint=()=>{let b='default'
const w=window.innerWidth
for(let i in lax.breakpoints){const px=parseFloat(lax.breakpoints[i])
if(px<=w){b=i}else{break}}
return b}
lax.updateElement=(o)=>{const{originalStyle,anchorTop,transforms,spriteData,el}=o
let r=anchorTop?anchorTop-lastY:lastY
const style={transform:originalStyle.transform,filter:originalStyle.filter}
for(let i in transforms){const transformData=transforms[i][currentBreakpoint]||transforms[i]["default"]
...
if(!t){continue}
t(style,v)}
if(spriteData){const[frameW,frameH,numFrames,cols,scrollStep]=spriteData
...
if(style.opacity===0){el.style.opacity=0}else{for(let k in style){el.style[k]=style[k]}}}
lax.update=(y)=>{if(lastY===y)return
lastY=y
lax.elements.forEach(lax.updateElement)}
return lax;})();if(typeof module!=='undefined'&&typeof module.exports!=='undefined')
module.exports=lax;else
window.lax=lax;})();